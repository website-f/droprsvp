    ...
    return (
        <>
            ...
            <div className="mx-auto w-full max-w-5xl flex-1 p-4">
                ...
                <form onSubmit={(e) => { e.preventDefault(); router.get('/admin/all-events', q ? { q } : {}, { preserveState: true }); }} className="mb-5 flex max-w-md gap-2">
                        ...
                        <tbody className="divide-y divide-border">
                            {events.data.length === 0 ? (
                                ...
                                <tr key={e.slug} className="hover:bg-muted/30">
                                    ...
                                    <td className="px-4 py-3 text-right">{e.status === 'published' && <Button asChild variant="ghost" size="sm"><a href={`/e/${e.slug}`} target="_blank" rel="noreferrer"><ExternalLink className="size-3.5" /></a></Button>}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
