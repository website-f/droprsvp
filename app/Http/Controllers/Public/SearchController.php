...
class SearchController extends Controller
{
    ...
    public function suggest(Request $request)
    {
        ...
        $adminHot = collect(preg_split('/,/', (string) Setting::get('trending_keywords', ''), -1, PREG_SPLIT_NO_EMPTY))
            ...
            ->filter(fn ($k) => $has($k))
            ->map(fn ($k) => ['label' => $k, 'url' => Url::path('all').'?q='.urlencode($k), 'source' => 'trending']);
        ...
        $systemHot = EventCategory::whereIn('id', $topCatIds)->get(['name', 'slug'])
            ...
            ->map(fn ($c) => ['label' => $c->name, 'url' => Url::path('all', $c->slug), 'source' => 'system']);
        ...
        $cities = $q === '' ? collect() : collect(Cities::all())
            ...
            ->take(5)->map(fn ($c) => ['label' => $c['name'], 'url' => Url::path($c['slug'])]);
